...
- Key components: `App.tsx` (routing/state, pane layout), `Pane.tsx` (one column: its own scroll container, headings, and scroll restoration), `Sidebar.tsx` (file list with flat/tree view, resizable, drag-and-drop reorder), `TreeView.tsx` (tree view with collapsible directories), `MarkdownViewer.tsx` (rendering + raw view toggle), `TocPanel.tsx` (table of contents, resizable), `GroupDropdown.tsx` (group switcher), `FileContextMenu.tsx` (shared kebab menu for file operations), `WidthToggle.tsx` (wide/narrow content width toggle)
- Custom hooks: `useSSE.ts` (SSE subscription with auto-reconnect), `useApi.ts` (typed API fetch wrappers), `useActiveHeading.ts` (scroll-based active heading tracking via IntersectionObserver), `usePanes.ts` (pane layout state + history entries for user-driven layout changes), `usePaneWidths.ts` (drag-to-resize column weights), `useScrollSync.ts` (locks the panes' scroll positions together), `useIsNarrow.ts` (matchMedia breakpoint for collapsing to one column)
- Utilities: `buildTree.ts` (converts flat file list to hierarchical tree with common prefix removal and single-child directory collapsing), `panes.ts` (pure pane-layout state transitions and URL encoding), `paneWidths.ts` (column weight maths and persistence), `scrollSync.ts` (heading-ordinal scroll position mapping), `dom.ts` (pane-scoped element lookup and scrolling), `fileRow.ts` (styling and click behaviour shared by the flat list and tree view)
...
- **Split panes**: Several files are shown side by side as equal-width columns, for comparing versions of the same document. Layout state is `{fileIds, focusIndex}` (`utils/panes.ts`, all transitions pure and returning the input object when nothing changed, so `App`'s render-time reconciliation cannot loop). Duplicates are allowed, so the same file can sit in two columns. `MAX_PANES` caps the count. Encoded in the URL as `?files=a,b,c&focus=1`; a single pane still emits the legacy `?file=id` form, and `?file=` is still parsed, so old links keep working. The layout is reconciled against the group's file list whenever groups change (panes whose file left the group are dropped).
- **Focused pane**: Exactly one column is focused. A plain sidebar click replaces the focused column's file; **Alt/Option+click** opens a new column (Cmd/Ctrl and Shift keep their browser meanings on the `<a>`, so Alt is the only free modifier — see `isAltLeftClick`). Clicking anywhere in a column focuses it (`onMouseDownCapture`, so toolbar buttons that stop propagation still focus). The focused column gets a `border-t-gh-accent` bar and its sidebar entry a left accent bar; every open file gets the highlighted sidebar treatment. Focus changes use `replaceState` (they happen constantly); opening and closing columns push history entries.
- **Pane widths**: Columns are `flex-grow` weights against `flex-basis: 0`, so proportions survive a window resize. A 4px handle on each column's left edge drags space between that column and its left neighbour only, clamped to `MIN_PANE_WIDTH`. Weights persist in localStorage but only apply to the same column count — a stored 3-column layout is ignored once there are 2 columns, rather than silently distorting them.
- **Scroll sync**: `ScrollSyncToggle` in the header (split view only) locks the columns together. Alignment is by **heading ordinal**, not heading ID: the panes are usually translations of one source, so heading text differs while structure is parallel. Position is "nth heading + fraction through that section" (`utils/scrollSync.ts`), falling back to a whole-document scroll ratio when the ordinal cannot be matched. `useScrollSync` caches heading offsets keyed on `scrollHeight` (a cheap invalidation signal for content and layout changes) and locks the pane that started scrolling as the driver for `DRIVER_LOCK_MS`, since programmatically setting `scrollTop` on the other panes fires their own scroll events.
- **Narrow windows**: Below `NARROW_BREAKPOINT` only the focused pane renders. The layout state is untouched, so widening restores every column. `PaneNavigator` appears in the header while collapsed — without it the hidden panes would be unreachable, because a sidebar click replaces the focused pane's file rather than moving between panes.
- **Duplicate heading IDs across panes**: Heading IDs come from the heading text (rehype-slug), so two panes showing the same document contain the same IDs. Every ID lookup must be scoped to the pane's container via `findElementById` — `document.getElementById` would always resolve to the leftmost column. This applies to `useActiveHeading`, `useScrollRestoration`, in-document hash links, and ToC clicks. `MarkdownViewer` was already safe because it scopes queries to `articleRef`.
- **Shared ToC panel**: The right-hand `TocPanel` shows the *focused* pane's outline (per-pane panels would be unreadable in a ~450px column). Each `Pane` keeps its own headings and publishes them, plus a container-scoped scroll callback, through `onFocusedTocChange` only while focused. Panel visibility follows the focused pane's per-file toggle in `tocOpenMap`.
- **Live-reload with panes**: `file-changed` bumps a per-file revision in `App` (`Record<fileId, number>`), so *every* column showing that file reloads. Each `Pane` captures its own scroll position off that revision change — the refetch is async, so the DOM still holds the pre-change content at that point.
...
- **Toolbar buttons in content area**: The toolbar column (ToC + Raw toggles) lives inside `MarkdownViewer.tsx`, positioned with `shrink-0 flex flex-col gap-2 -mr-4 -mt-4` to align with the header. In split view it also renders `ClosePaneButton` — deliberately distinct from `CloseFileButton`: closing a pane removes the column, closing a file removes it from the group.
- **Scrolling to a heading**: Use `scrollElementToTop(container, el)` rather than `el.scrollIntoView()`. It drives the pane container's own `scrollTo`, offsets the sticky file-name bar so the heading is not hidden behind it, and honours `prefers-reduced-motion`.
...
- **localStorage conventions**: All keys use `mo-` prefix (e.g., `mo-sidebar-width`, `mo-sidebar-viewmode`, `mo-sidebar-tree-collapsed`, `mo-theme`). Read patterns use `try/catch` around `JSON.parse` with fallback defaults. Pane-related keys are `mo-pane-widths` and `mo-scroll-sync`. Scroll context lives in sessionStorage under `mo-scroll-context`, suffixed per pane (`mo-scroll-context:1`, …) with pane 0 keeping the unsuffixed key.
